...
var defs4 = map.select("defs");
var path = d3.geoPath();

defs4
  .append("path")
  .attr("id", "nation")
  .attr("d", path(topojson.feature(usa, usa.objects.nation)));
...
function updateMap(fips) {
  nodes4.attr("stroke", (county) => {
    if (county.id == fips) return "blue";
    else return "none";
  });
}
